...
void countsortforradix (long long int v[],int n,int p,int baza)
{
...
void radixsort (long long int v[],int n,int baza)
{
    int m = getMax(v, n);
for(int p=1;m/p>0;p*=baza)
    countsortforradix(v,n,p,baza);
}
...
int medianof3(long long int v[],int mini, int maxi) {
    int pivot=0, mid = (mini + maxi) / 2;
    if (v[mini] > v[mid])
        pivot=mini;
    else pivot=mid;
    ...
    swapi(v[pivot], v[maxi]);

    return pivot;
}

void quicksort2(long long int v[],int mini,int maxi)
{
    if(mini<maxi)
    {
        int piv=medianof3(v,mini,maxi);
        quicksort(v, mini,piv - 1);
        quicksort(v, piv + 1, maxi);
...
fnct functions[]={bubblesort,
    countsort,radixsort,radixsort,mergesort,quicksort,quicksort2};
string fctname[]={"bubblesort",
"countsort","radixsort cu baza 10","radixsort cu baza 2","mergesort","quicksort cu ultimul element ca mediana","quicksort cu mediana din 3"};
for(i=1;i<=t;i++)
{
    f>>n>>maxi;
...
//sort(v1+1,v1+n+1);
if(w==2)functions[w](v2,n,10);
else if(w==3)functions[w](v2,n,2);
else if(w>=4)functions[w](v2,1,n);
    else
